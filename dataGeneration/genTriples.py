...
dp=Dataprocessor_test(T5Tokenizer.from_pretrained("../GeneratorModel/out"),"")
tokenizer = T5Tokenizer.from_pretrained("../GeneratorModel/out")
model = T5ForConditionalGeneration.from_pretrained("../GeneratorModel/out-wikidata-labels/checkpoint-150000")
...
for question in data:
    try:
        ...
        print(input)
        print(question["sparql_wikidata"])
        ...
        model_out = tokenizer.decode(out[0], skip_special_tokens=True)
        query_triples,query_patterns=process_query(prefixes+query)
        question["model_out"]=model_out
        question["triples"]=query_triples
