...
def generate_answers(query_str):
    try:
        sparql = SPARQLWrapper(
                    "https://freebase.data.dice-research.org/sparql/",
...
grail_ans=json.load(open("../sexpfreebase.json","r",encoding="utf-8"))
grail_qald=json.load(open("../qa-data/GrailQA_v1.0/grail_dev_qald_corrected.json"))

for qe in tqdm(grail_qald["questions"]):
    if str(qe["id"]) in grail_ans:
        answers=grail_ans[str(qe["id"])]["candidates"]
        try:
            query=answers[0]["logical_form"]
            query=lisp_to_sparql(query)

            #lisp_to_sparql(st)query)
            ans=generate_answers(query)
        except:
            print("failed to parse")
            ans = {'head': {'vars': ['result']}, 'results': {'bindings': []}}
        print(ans)
    else:
    ...
    bindings=[]
    if len(qe["answers"])>0:
        qe["answers"][0]["head"]=ans["head"]
        qe["answers"][0]["results"]["bindings"]=ans["results"]["bindings"]

json.dump(grail_qald,open("sexp_out.json","w",encoding="utf-8"), ensure_ascii=False,indent=4)
#st="(AND opera.opera_designer_role (JOIN (R opera.opera_designer_gig.design_role) (JOIN opera.opera_production.designers m.0pm2fgf)))"
#sp=lisp_to_sparql(st)
#print(sp)
